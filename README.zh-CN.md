...
- [`demo/index.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html) 试验台：**四代引擎同时在跑，每一代都用它当年展示时的那组参数** —— 0.1.0 把边缘弯过去，0.2.0 把这件事做便宜，0.3.1 去掉阶梯，0.4.0 给边缘明暗、而且默认值不再拘谨，0.5.0 让这块玻璃在元素变尺寸、凝出来的时候都不熄灭。这些数字取自各个 tag 上的这个页面，不是引擎的 `DEFAULTS` —— 默认值一向是刻意保守的，而一版**长什么样**，看的是它的试验台当时摆出来的样子：0.1.0 是 10px 玻璃上一道 25px 倒角，到 0.3.1 是 19 和 28，现在是 59 和 37。滑杆只管当前这张。（`demo/legacy/` 放着旧引擎的逐字副本，各挂各的全局名、自定义属性和滤镜 id 前缀，所以四代能在一页上同时渲染 —— 为对比冻结的，不是受支持的构建。）
...
| `settle` | 0 | ms。合并尺寸变化：最后一次变化之后 `settle` ms 重算一次；期间玻璃不掉，旧贴图被拉伸盖住新尺寸。设 0 走实时：一阵变化的第一次在那一帧画出来之前就重算好，之后每 90 ms 最多重算一次，停下来再补最后一次。一行一行长的东西（聊天气泡）用实时；正在被拖拽的框才用 `settle` |
...
7. **滤镜。** `feImage`（贴图）→ `feGaussianBlur`（磨砂）→ 一道把模糊结果钉成不透明的 `feColorMatrix`（引用滤镜拿到的背景到元素盒子就断了，模糊会把最外几行淡成半透明；下面三通道色散相加在半透明像素上会把颜色加三倍——凝出来的头一两帧、边缘还在采自己边界的时候，就是一圈白发丝线）→ 内层 `feDisplacementMap` → 倒角圈遮罩的 `feGaussianBlur`（`smooth`）→ 外层 `feDisplacementMap`（一次，或色散时每个颜色通道一次再用 `feComposite arithmetic` 相加）→ 边缘光叠在上面。滤镜区域就是元素自己的盒子（`objectBoundingBox` 0 0 1 1），贴图拉伸填满它，所以元素长大时玻璃不掉、等下一次重算；`color-interpolation-filters="sRGB"` 让 128 真的等于零。
8. **`backdrop-filter: url(#id)`** 负责剩下的事：实时，对元素背后的一切。
9. **不熄灭。** 0.4.0 之前滤镜区域钉死在建图时的尺寸，聊天气泡一长行就整条画成没滤镜，`settle` 用一段普通模糊加渐入把它盖住——结果是流式回复每停顿一下就镜片→磨砂→镜片地闪一次。现在变尺寸＝先拉伸、再一帧换成重算好的滤镜；新建的滤镜第一帧就画得对（在 Chrome 152 里逐帧验过），没有东西需要遮。
...
- 贴图在主线程上算（逐像素循环加一次 PNG 编码）。实时模式下变尺寸的元素每 90 ms 最多重算一次——一阵变化的第一次是同步的，就在 ResizeObserver 回调里，所以显示新尺寸的那一帧已经是新贴图；`settle` 能把它减到停下后一次。贴图封顶约 32 万像素。
...
- 形状变形的元素（药丸长成卡片）：变形期间自己把 `--hyalite` 写成 `blur(你的模糊px)`，落定后再带 `materialize` 挂上。已挂上的元素变尺寸时玻璃不掉（先拉伸、再重算），一行一行长的文字正合适；药丸变卡片那种会每 90 ms 重算一次、看着像橡皮，所以落定再挂。
- 贴图是 `data:` URL，严格的 CSP 需要 `img-src data:`。

## 转载

demo 页面自带落款——大字上方的仓库地址、每块玻璃底边刻着的签名、页脚的那枚胶囊——所以录屏也带着回来的路。转发录屏时，请让其中一处留在画面里，或者在文案里写上仓库链接和作者 VII-Cae（VII）。代码是 MIT：随便用、随便发，把声明留着就行。
