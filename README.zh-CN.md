...
- [`demo/index.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html) 试验台：**四代引擎同时在跑，每一代都用它当年展示时的那组参数** —— 0.1.0 把边缘弯过去，0.2.0 把这件事做便宜，0.3.1 去掉阶梯，0.4.0 给边缘明暗、而且默认值不再拘谨。这些数字取自各个 tag 上的这个页面，不是引擎的 `DEFAULTS` —— 默认值一向是刻意保守的，而一版**长什么样**，看的是它的试验台当时摆出来的样子：0.1.0 是 10px 玻璃上一道 25px 倒角，到 0.3.1 是 19 和 28，现在是 59 和 37。滑杆只管 0.4.0 那张。（`demo/legacy/` 放着旧引擎的逐字副本，各挂各的全局名、自定义属性和滤镜 id 前缀，所以四代能在一页上同时渲染 —— 为对比冻结的，不是受支持的构建。）
...
| 参数 | 默认 | 含义 |
|---|---|---|
| `bevel` | 37 | 边缘弯折带的宽度，px。封到**短边的一半** —— 不再封到圆角，所以圆形能一路弯到中心 |
| `thickness` | 59 | 玻璃厚度，px。决定边缘把背景往里拉多远 |
...
| `shade` | 0.46 | 边缘压暗多少，0–2。一个数管两件事，比例是调出来的：焦散（边缘放大背景，能量被摊薄）和菲涅尔透射损失 |
| `rim` | 1.76 | 边缘送回多少光，0–4：一层宽的菲涅尔泛光，加一道收紧的高光线。这一份在**滤镜里** —— 像素级锐利的那道外描边看 `edge` |
| `edgeW` | 8 | px —— 明暗和泛光往里够多远。故意用绝对像素（见[原理](#原理)规则 3） |
| `sat` | 0.86 | 倒角圈内的饱和度，0–3。折叠和色散都会把边缘的颜色搅浑，压到 1 以下能洗干净，中心一概不碰。设 1 直接跳过它要的那七个滤镜节点 |
...
**贴图**取决于几何 ＋ `bevel` ＋ `thickness` ＋ `slope` ＋ `shape` ＋ `shade` ＋ `rim` ＋ `edgeW` ＋ `light` —— 位移场和边缘剖面是由这些算出来的，少一个都不行；**滤镜**在这之上再加 `blur`、`dispersion`、`sat`、`smooth` 和 `self`，这几个都不碰贴图。
...
3. **折叠，可选。** 位移的衰减斜率封顶在 `slope` px/px。到 1 采样点就不动了（无限拉伸），超过 1 画面开始镜像 —— 同一块背景出现两次。0.4.0 之前这是禁止的、死封在 0.85；现在它是个参数，而且出厂默认（2.7）**就是折叠的**，因为「液态」的观感正是从这儿来的。它仍然是一道悬崖：折叠区的拉伸没有上界，所以要把它关在一条窄倒角里，让 `blur` 和 `dispersion` 去盖。
4. **不出阶梯。** Chromium 对被弯折的画面做的是最近邻采样——Skia 的位移效果写死了 `kNearest`（[skbug 40045448](https://issues.skia.org/40045448)）。斜率 0.85 就是 6.7 倍拉伸，边缘上每个源像素都被复制成 6.7px 的方块，玻璃后面任何一条硬边都会变成阶梯。贴图再准也治不了采样器，所以 —— 只在位移场不折叠时 ——把位移场拆成两段拉伸相等（各约 2.6 倍）的位移，两段合起来**严格**等于单段的场（内层那张表是外层的反函数，不是简单对半）。两段之间加一道 `smooth` 像素的模糊，用倒角圈做遮罩，先把内层那段的阶梯化掉，再让外层去拉伸。6.7px 的满对比阶梯变成约 2.6px、对比只剩一小部分；中心区一点不动。 一旦 `slope` 越过 1，这个拆分就不存在了（位移场不可逆），`smooth` 会被忽略，段数退回一段。
...
Apple 的 Liquid Glass（WWDC25）给了「玻璃该弯光而不是散射光」这个想法。圆角矩形 SDF → 折射 → 位移贴图 → `feDisplacementMap` 这条路已有不少项目走过，[kube.io](https://kube.io/blog/liquid-glass-css-svg/) 的物理推导最清楚。Hyalite 自己的实现取舍是：从位移场推出来（而不是画上去）的边缘明暗、把折叠变成可选、大圆方向场、四角各自半径、按真实元素尺寸建图、settle / materialize 行为、私有的渐入克隆，以及一套能在真实页面上活下来的 watch/attach API。
