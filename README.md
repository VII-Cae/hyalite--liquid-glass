...
- [`demo/index.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html) — **the previous engine next to the current one**, on the same geometry, so the difference between the cards is the engine and not the parameters: 0.3.1 bends the edge at one brightness throughout, 0.4.0 shades it. Drag, swap, tune the nine knobs that move the look most, switch to a grid or load your own photo. (`demo/hyalite-0.3.1.js` is a frozen copy kept for that comparison — do not depend on it.)
