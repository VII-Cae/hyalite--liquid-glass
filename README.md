...
- [`demo/index.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html) — **all four engines at once, each on the defaults it shipped with**: 0.1.0 bends, 0.2.0 does it cheaply, 0.3.1 loses the staircase, 0.4.0 shades the edge and stops being shy about it. That is the material each version actually came with, which is the honest comparison — the first three are a 16px bevel over 10px of glass with a 3px frost; the current one is 37 over 59 and clear in the middle. The sliders drive the 0.4.0 card only. (`demo/legacy/` holds verbatim copies of the older engines — each on its own global, custom property and filter-id prefix so four can render on one page. Frozen for the comparison, not supported builds.)
