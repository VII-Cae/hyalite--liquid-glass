...
- [`demo/index.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html) — **all four engines at once**, every one of them driven by the same geometry, so what differs between the cards is the version and not the parameters: 0.1.0 bends, 0.2.0 does it cheaply, 0.3.1 loses the staircase, 0.4.0 shades the edge. Drag them, tune the nine knobs that move the look most, switch to a grid or load your own photo. (`demo/legacy/` holds verbatim copies of the older engines, each on its own global and its own custom property so four can render on one page — frozen for the comparison, not supported builds.)
