...
<sub>这张图由 <code>demo/lab-engine.js</code>（0.4.0 试验引擎）渲染：0.3.1 做的是折射，还没有边缘明暗。<a href="demo/card.html">卡片的源码</a>。</sub>
...
- [`demo/cases.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/cases.html) 自检页：不对称圆角、椭圆圆角、CSS 相邻角规则、共用滤镜的双胞胎、尺寸分桶、把贴图读回来验四分之一对称、圆形上不出接缝的方向场、厚透镜上被封住的色散分离、流式长高的气泡，以及把封顶后的值真读回来核对（而不是「没抛异常就算过」）。
...
| `blur` | 3 | 中心的磨砂，px |
| `dispersion` | 0.05 | 色散，0–0.5。设 0 只做一次位移，明显更省。它是**位移的百分比**，所以最终的通道分离量封在 3px 以内：很厚的透镜得到的是一条彩边，而不是放射状的彩虹 |
| `rim` | 0.45 | 随几何走的边缘光，0–4。0 关掉 |
| `light` | −145 | 边缘光从哪个方向来，度。0 是正上方，正值顺时针转。默认值落在左下，和向下的投影相抵，玻璃读起来是「浮着」而不是「被顶灯照着」——目测定的，不是推出来的 |
| `smooth` | 1 | 像素。用来藏住 Chromium 沿边缘的最近邻阶梯的那道模糊（见 [不出阶梯](#原理)）。它加在两次位移**之间**，且只作用于倒角那一圈，中心永远看不到它。设 0 回到单次位移：更省，阶梯也回来 |
...
工程实现：Claude Fable 5.1（0.1.0，以及 0.3.0：藏住 Chromium 最近邻阶梯的两段位移、`smooth`）与 Claude Opus 5（0.2.0：两级缓存与尺寸分桶、圆角的 CSS 相邻角规则、四分之一对称、`light`、`force`，以及一个把值真读回来核对的自检页；0.3.1：把圆形修好的方向场封顶，以及不让厚透镜变成彩虹的色散封顶），都来自 Anthropic，都与 VII-Cae 结对完成；VII-Cae 负责方向、每一版的目测验收和每一个参数的手调。
