...
- [`demo/index.html`](https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html) — **all four engines at once, each on the settings its own release was shown at**: 0.1.0 bends, 0.2.0 does it cheaply, 0.3.1 loses the staircase, 0.4.0 shades the edge and stops being shy about it. Those numbers come from this page at each tag, not from the engines' `DEFAULTS` — the defaults were always deliberately conservative, and what a version *looked* like is what its playground displayed: 25px of bevel over 10px of glass in 0.1.0, 28 over 19 by 0.3.1, 37 over 59 now. The sliders drive the 0.4.0 card only. (`demo/legacy/` holds verbatim copies of the older engines — each on its own global, custom property and filter-id prefix so four can render on one page. Frozen for the comparison, not supported builds.)
